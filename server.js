...
const mysql = require('mysql'); // mysql clásico
...
app.get('/api/gorras', async (req, res) => {
  try {
    ...
    res.json(rows);
  } catch (e) { console.error('GET /api/gorras error:', e); res.status(500).json({ error: 'Error al obtener gorras' }); }
});
...
app.get('/api/gorras/:id', async (req, res) => {
  try {
    ...
    res.json(rows[0]);
  } catch (e) { console.error('GET /api/gorras/:id error:', e); res.status(500).json({ error: 'Error interno al consultar el producto', detail: e.message }); }
});
...
app.post('/api/gorras', verificarToken, upload.single('imagen'), async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Gorra creada correctamente', id: result.insertId, gorra: { id: result.insertId, Nombre, precio, imagen, descripcion } });
  } catch (e) { console.error('POST /api/gorras error:', e); res.status(500).json({ error: 'Error al crear gorra' }); }
});
...
app.put('/api/gorras/:id', verificarToken, upload.single('imagen'), async (req, res) => {
  ...
  } catch (e) { console.error('PUT /api/gorras/:id error:', e); res.status(500).json({ error: 'Error al actualizar' }); }
});
...
app.delete('/api/gorras/:id', verificarToken, async (req, res) => {
  ...
  } catch (e) { console.error('DELETE /api/gorras/:id error:', e); res.status(500).json({ error: 'Error al eliminar' }); }
});
